...
def backup_db():
    ...
    try:
        global username, password, db
        sql_path = filedialog.asksaveasfilename(title="Choose Where to Save Database", confirmoverwrite=True, initialdir=default_path, defaultextension=".sql", filetypes=[("SQL files", "*.sql")])
        if sql_path:
...
def faculty_write_to_csv():
    default_path = os.getcwd()
    csv_path = filedialog.asksaveasfilename(initialdir=default_path, defaultextension=".csv", filetypes=[("CSV files", "*.csv")])
    if csv_path:
        try:
...
search_button = tkb.Button(faculty_options_frame, text="Search", compound=tk.LEFT, command=search_button_click,padding=(1,1))
...
search_school_button = tkb.Button(school_options_frame, text="Search", compound=tk.LEFT, command=search_school_button_click,padding=(1,1))
...
search_payroll_button = tkb.Button(payroll_options_frame, text="Search", compound=tk.LEFT, command=search_payroll_button_click,padding=(1,1))
...
search_positions_button = tkb.Button(positions_options_frame, text="Search", compound=tk.LEFT, command=search_positions_button_click,padding=(1,1))
...
search_coord_button = tkb.Button(coord_options_frame, text="Search", compound=tk.LEFT, command=search_coord_button_click,padding=(1,1))
...
search_dept_fac_button = tkb.Button(dept_fac_options_frame, text="Search", compound=tk.LEFT, command=search_dept_fac_button_click,padding=(1,1))
...
faculty_sort_desc = tkb.Checkbutton(faculty_options_frame, text="Desc", variable=faculty_sort_desc_var, bootstyle="info-round-toggle")
faculty_sort_desc.grid(row=1, column=1, padx=(0, 10), sticky=tk.E)

faculty_sort_button = tkb.Button(faculty_options_frame, text="Go", command=lambda: faculty_sort_by(faculty_sort_entry.get(), faculty_sort_desc_var.get()),padding=(1,1))
...
school_sort_desc = tkb.Checkbutton(school_options_frame, text="Desc", variable=school_sort_desc_var, bootstyle="info-round-toggle")
...
payroll_sort_desc = tkb.Checkbutton(payroll_options_frame, text="Desc", variable=payroll_sort_desc_var, bootstyle="info-round-toggle")
...
positions_sort_desc = tkb.Checkbutton(positions_options_frame, text="Desc", variable=positions_sort_desc_var, bootstyle="info-round-toggle")
...
coord_sort_desc = tkb.Checkbutton(coord_options_frame, text="Desc", variable=coord_sort_desc_var, bootstyle="info-round-toggle")
...
dept_fac_sort_button = tkb.Button(
    dept_fac_options_frame, text="Go", command=lambda: dept_fac_sort_by(dept_fac_sort_entry.get(), dept_fac_sort_desc_var.get()), padding=(1, 1))
...
positions_info_header = tkb.Label(positions_frame, text="Simple CRUD", font=("Times New Roman", 25, "bold", "underline"))
...
dept_fac_commands = [add_record_dept_fac, update_record_dept_fac, delete_record_dept_fac, dept_fac_write_to_csv, backup_db]
...
school_treeview = tkb.Treeview(school_table_frame, columns=("School Number", "School Name"), show="headings", bootstyle="info")
