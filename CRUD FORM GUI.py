...
root.geometry("1400x500")  # Adjust the initial size of the window
root.withdraw()

root.maxsize(1400,500)
root.minsize(1400,500)
# Define function for creating and handling login form
def show_login_form():
    login_window = tk.Toplevel(root)
    login_window.title("MySQL Login")
    login_window.geometry("250x100")
    login_window.maxsize(250,100)
    login_window.minsize(250,100)
    ...
    tk.Label(login_window, text="MySQL Username:").grid(row=0, column=0, pady=(10,10))
    username_entry = tk.Entry(login_window)
    username_entry.grid(row=0, column=1)

    tk.Label(login_window, text="MySQL Password:").grid(row=1, column=0)
    password_entry = tk.Entry(login_window, show="*")
    password_entry.grid(row=1, column=1)

    def login_callback():
        login(username_entry.get(), password_entry.get(), login_window)

    login_button = tk.Button(login_window, text="Login", command=lambda: login(username_entry.get(), password_entry.get(), login_window))
    login_button.grid(row=2, column=1, sticky=tk.W)

    def clear_fields():
        username_entry.delete(0, tk.END)
        password_entry.delete(0, tk.END)

    clear_button = tk.Button(login_window, text="Clear", command=clear_fields)
    clear_button.grid(row=2, column=1, columnspan=2, pady=5, padx=35)
...
def login(username, password, window):
    try:
        conn = mysql.connector.connect(
            host='localhost',  # typically 'localhost' or a specific IP address
            user=username,
            password=password,
            database='MAMANSKI'  # specify the database to connect to
        )
        ...
        messagebox.showerror("Login Failed", "Invalid Credentials! Could not connect to MySQL: {}".format(err))
...
crud_info_header.grid(row=0, column=0, pady=(20, 12), padx=10, sticky=(tk.W, tk.E))
